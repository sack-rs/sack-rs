...
pub struct Sack<T: Token, C: Token, I: SackLike<T,C>> {
    pub t: T, // The token by which I know myself, and by which my parents know me
    pub c: PhantomData<C>, // The fully qualified typesystem of all possible nestings within the child's sacks. the descendants of the children
    pub i: I, // The storage implementation of this (non-nested) sack, which must implement sacklike functionality over C,D
}

// impl Into<Sack<(),(), (), ()>> for () {}

pub trait SackLike<T:Token, C:Token>: Token + Into<Sack<T, C, Self>> {
    fn len(&self) -> isize;
...
impl<T: Token, C: Token, I: SackLike<T, C>> Token for Sack<T, C, I> {}
